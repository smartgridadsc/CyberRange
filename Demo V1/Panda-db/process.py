...
class GenNetwork:
    ...
    def create_custom_line(self, net, from_bus, to_bus, length_km, choice, cable_dict):
        ...
        lidx = pp.create_line_from_parameters(net=net, from_bus=from_bus, to_bus=to_bus, length_km=length_km,
                                       r_ohm_per_km=temp[0],
    ...
    def generate(self):
        ...
        for vlevel in self.conductingEquipment.keys():
            ...
            for conduct in self.conductingEquipment[vlevel]:  # iterate power grid components
                ...
                if conduct['type'] == "CBR":
                    ...
                    # self.create_line_with_switch(self.net, first_bus=self.node_to_busid[conduct["node1"]], last_bus=self.node_to_busid[conduct["node2"]],
                    #                              length_km=0.02, std_type=std_types[0])
                    self.create_line_with_switch_with_status(self.net, first_bus=self.node_to_busid[conduct["node1"]],
                                                 last_bus=self.node_to_busid[conduct["node2"]], length_km=0.02, std_type=std_types[0],
                ...
                # load
                    ...
                    lidx = pp.create_line(net=self.net, from_bus=temp_bus, to_bus=temp_bus,
                                   length_km=self.input_dict[conduct["type"]][self.get_path(conduct)][1],
                                   std_type=std_types[1])
                    print("LINE ID=" + str(lidx) + " From=" + str(temp_bus) + " To=" + str(temp_bus))
                ...
                #current transformer
                    ...
                    lidx = pp.create_line(net=self.net, from_bus=self.node_to_busid[conduct["node1"]],
                                   to_bus=self.node_to_busid[conduct["node2"]], length_km=0.1, std_type="NAYY 4x150 SE")
                    print("LINE ID=" + str(lidx) + " From=" + str(self.node_to_busid[conduct["node1"]]) + " To=" + str(self.node_to_busid[conduct["node2"]]))
        ...
        for trans in self.root.findall('.//' + self.add_xmlns("PowerTransformer")):
              ...
              print("TTTTTTTT" + str(self.node_to_busid[terminals[0].get("connectivityNode")]))
              print("TTTTTTTT" + str(self.node_to_busid[terminals[1].get("connectivityNode")]))
              print(self.node_to_busid)
              pp.create_transformer(net=self.net, hv_bus=self.node_to_busid[terminals[0].get("connectivityNode")],
                                          lv_bus=self.node_to_busid[terminals[1].get("connectivityNode")],
                                           std_type="40 MVA 110/10 kV")
              # for trans in self.transformerWinding.keys():
              #     #volt = self.transformer[trans]["voltage"]
              #     for winding in self.transformerWinding[trans]:
              #         if winding["type"] == "PTW":
              #               # temp_bus = pp.create_bus(net=self.net, vn_kv=volt)
              #               # print(temp_bus)
              #               # self.create_custom_line(net=self.net, from_bus=self.node_to_busid[conduct["node1"]],
              ...
              #               if count == 0:
              #                   temp_bus = pp.create_bus(net=self.net, vn_kv=volt)
              #                   print("hhhhhhh" +str(winding))
              #                   print(self.input_dict)
              #                   self.create_custom_line(net=self.net, to_bus=self.node_to_busid[conduct["node1"]],
              #                                           from_bus=temp_bus,
              #                                           length_km=self.input_dict[winding["type"]][self.get_path(winding)][1],
              #                                           choice=self.input_dict[winding["type"]][self.get_path(winding)][2],
              #                                           cable_dict=self.input_dict["CABLE"])
              #
              #                   pp.create_transformer(net=self.net, hv_bus=self.node_to_busid[terminals[0].get("connectivityNode")],
              #                                         lv_bus=self.node_to_busid[terminals[1].get("connectivityNode")])
              ...
              # if self.node_to_busid[terminals[0].get("connectivityNode")]==9:
              #       #self.create_line_with_switch(self.net, first_bus=1, last_bus=9, std_type="NAYY 4x120 SE", length_km="0.02")
              #       pp.create_line(self.net, from_bus=1, to_bus=9, std_type="NAYY 4x120 SE", length_km="0.02")
              #       pp.create_transformer(net=self.net, hv_bus=self.node_to_busid[terminals[0].get("connectivityNode")],
              #                                         lv_bus=self.node_to_busid[terminals[1].get("connectivityNode")], std_type="40 MVA 110/10 kV")
              #       #self.create_line_with_switch(self.net, first_bus=21, last_bus=11, std_type="NAYY 4x120 SE", length_km="0.02")
              #       pp.create_line(self.net, from_bus=21, to_bus=11, std_type="NAYY 4x120 SE", length_km="0.02")
              # if self.node_to_busid[terminals[0].get("connectivityNode")]==29:
              #       #self.create_line_with_switch(self.net, first_bus=8, last_bus=29, std_type="NAYY 4x120 SE", length_km="0.02")
              #       pp.create_line(self.net, from_bus=8, to_bus=29, std_type="NAYY 4x120 SE", length_km="0.02")
              #       pp.create_transformer(net=self.net, hv_bus=self.node_to_busid[terminals[0].get("connectivityNode")],
              #                                         lv_bus=self.node_to_busid[terminals[1].get("connectivityNode")],std_type="40 MVA 110/10 kV")
              #       #self.create_line_with_switch(self.net, first_bus=39, last_bus=20, std_type="NAYY 4x120 SE", length_km="0.02")
              #       pp.create_line(self.net, from_bus=39, to_bus=20, std_type="NAYY 4x120 SE", length_km="0.02")
