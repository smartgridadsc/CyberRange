...
for (auto& el:jf_cp["CPMapping"].items())
    ...
    //for time stamp
    vector<long long> PTOC51_store_time, PTOV59_alarm_store_time, PTOV59_trip_store_time, PTUV27_alarm_store_time, PTUV27_trip_store_time; //long long because the time_since_epoch is a long number
    ...
    if (jf_t.contains("PTOV59"))
    {
        ...
        {
            for(auto& inner_el : el.value()["AlarmThreshold"]["Limit_p.u."].items()) // for nested JSON with {} due to different type for PTOV59 tag and Alarmthreshold tag
            {
                PTOV59_alarm_limit_list.push_back(stod(inner_el.value().get<string>())); //.get<string> convert basic_json type to string, stod converts string to double
            }
