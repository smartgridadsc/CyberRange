...
void updateFunction()
{
    ...
    IedServer_updateFloatAttributeValue(
            iedServer, IEDMODEL_MEAS_MMXU9_TotW_mag_i, flt);
    float read = IedServer_getFloatAttributeValue(
            iedServer, IEDMODEL_MEAS_MMXU9_TotW_mag_i);
...
void run_ied()
{
    ...
    try {
        ...
        db_conn.connect("test_db", "localhost", "testuser", "password", 3306);
    }
    catch (mysqlpp::Exception e) {
...
{
    char *mapping_file_name = "Load9.iid";
    interval = 1000;
