...
void updateFunction()
{
    ...
    try {
        const char* querystring = "SELECT value FROM fm ORDER BY LENGTH(name),name";
        mysqlpp::Query query = db_conn.query(querystring);
        result = query.store();
    }
    catch (mysqlpp::Exception e) {
    ...
    IedServer_updateTimestampAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU1_Hz_t, &iecTimestamp);
    IedServer_updateFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU1_Hz_mag_f, flt);
    float read = IedServer_getFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU1_Hz_mag_f);
    printf("MMXU1_Hz_mag_f (fm_0) = %f\n", read);

    resultstring = ((result.at(1)))[0].c_str();
    flt = std::stof(resultstring);
    IedServer_updateTimestampAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU2_Hz_t, &iecTimestamp);
    IedServer_updateFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU2_Hz_mag_f, flt);
    read = IedServer_getFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU2_Hz_mag_f);
    printf("MMXU2_Hz_mag_f  (fm_1) = %f\n", read);

    resultstring = ((result.at(2)))[0].c_str();
    flt = std::stof(resultstring);
    IedServer_updateTimestampAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU3_Hz_t, &iecTimestamp);
    IedServer_updateFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU3_Hz_mag_f, flt);
    read = IedServer_getFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU3_Hz_mag_f);
    printf("MMXU3_Hz_mag_f  (fm_2) = %f\n", read);

    resultstring = ((result.at(3)))[0].c_str();
    flt = std::stof(resultstring);
    IedServer_updateTimestampAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU4_Hz_t, &iecTimestamp);
    IedServer_updateFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU4_Hz_mag_f, flt);
    read = IedServer_getFloatAttributeValue(
            iedServer, IEDMODEL_Frequency_MMXU4_Hz_mag_f);
    printf("MMXU4_Hz_mag_f  (fm_3) = %f\n", read);
...
void run_ied()
{
    ...
    try {
        ...
        db_conn.connect("test_db2", "localhost", "testuser", "password", 3306);
    }
    catch (mysqlpp::Exception e) {
...
{
    char *mapping_file_name = "ied20scl.icd";
    interval = 1000;
