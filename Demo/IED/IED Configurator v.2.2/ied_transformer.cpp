...
void updateFunction()
{
    ...
    bool status;
    try {
        const char* querystring = "SELECT value FROM transformer_temp WHERE name=\"transformer_temp_0\"";
        mysqlpp::Query query = db_conn.query(querystring);
        mysqlpp::StoreQueryResult result = query.store();
        std::string resultstring = (*(result.begin()))[0].c_str();
        flt = std::stof(resultstring);
        if (flt > TEMP_THRESHOLD) {
            status = false;
        }
        else {
            status = true;
    ...
    IedServer_updateTimestampAttributeValue(
            iedServer, IEDMODEL_PROT_PTTR1_Mod_t, &iecTimestamp);
    IedServer_updateBooleanAttributeValue(
            iedServer, IEDMODEL_PROT_PTTR1_Mod_stVal, status);
    bool read = IedServer_getBooleanAttributeValue(
            iedServer, IEDMODEL_PROT_PTTR1_Mod_stVal);
    printf("PTTR1_Mod_stVal (transformer_temp_0) = %i\n\tTemperature reading = %f\n", read,flt);

    try {
        const char* querystring = "SELECT value FROM transformer_temp WHERE name=\"transformer_temp_1\"";
        mysqlpp::Query query = db_conn.query(querystring);
        mysqlpp::StoreQueryResult result = query.store();
        std::string resultstring = (*(result.begin()))[0].c_str();
        flt = std::stof(resultstring);
    ...
    IedServer_updateTimestampAttributeValue(
            iedServer, IEDMODEL_PROT_PTTR2_Mod_t, &iecTimestamp);
    IedServer_updateBooleanAttributeValue(
            iedServer, IEDMODEL_PROT_PTTR2_Mod_stVal, status);
    read = IedServer_getBooleanAttributeValue(
            iedServer, IEDMODEL_PROT_PTTR2_Mod_stVal);
    printf("PTTR2_Mod_stVal (transformer_temp_0) = %i\n\tTemperature reading = %f\n", read,flt);
...
{
    char *mapping_file_name = "ied19scl.icd";
    interval = 1000;
