...
static ControlHandlerResult controlHandlerForBinaryOutput(ControlAction action, void *parameter, MmsValue *value, bool test)
{
    ...
    if (parameter == IEDMODEL_LogicalDevice_GGIO3_SPC1) {
        ...
        if (MmsValue_getBoolean(value))
            querystring = "UPDATE line_cb SET value=1 WHERE name=\"line_cb_12\"";
        else
            querystring = "UPDATE line_cb SET value=0 WHERE name=\"line_cb_12\"";
...
void updateFunction()
{
    ...
    float flt;
    try {
        const char* querystring = "SELECT value FROM line_loading WHERE name=\"line_loading_21\"";
        mysqlpp::Query query = db_conn.query(querystring);
    ...
    try {
        const char* querystring = "SELECT value FROM line_load_percent WHERE name=\"line_load_percent_21\"";
        mysqlpp::Query query = db_conn.query(querystring);
        mysqlpp::StoreQueryResult result = query.store();
        std::string resultstring = (*(result.begin()))[0].c_str();
        flt = std::stof(resultstring);
    ...
    try {
        const char* querystring = "SELECT value FROM line_cb WHERE name=\"line_cb_12\"";
        mysqlpp::Query query = db_conn.query(querystring);
        mysqlpp::StoreQueryResult result = query.store();
        std::string resultstring = (*(result.begin()))[0].c_str();
        flt = std::stof(resultstring);
...
{
    char *mapping_file_name = "ied7scl.icd";
    interval = 1000;
