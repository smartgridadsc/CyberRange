...
from process import GenNetwork
...
class Network():
    ...
    def set_plc_data_from_plc(self, plc_data):
        ...
        print("New net actuator values:")
        for i in range(len(self.net.switch["closed"])):
            st = self.net.switch["closed"][i]
            #print(f"switch[{i}] value: {st}")
    ...
    def set_plc_data_from_plc_round(self, plc_data, round):
        ...
        #Currently it is assumed that the number of actuators (i.e., CBs) can be found in extra_config, and is static.
    ...
    def get_data_for_plc(self):
        ...
        if Constants.IS_ATTACK_MODE:
            ...
            for attack_instance_name, attack_setting_dict in self.attack_setting.items():
                ...
                print(f"Modifying data for {attack_instance_name} ...")
                ...
                for instance_id, value_queue in attack_setting_dict.items():
                    ...
                    print(f"--- Modifying data for {instance_id} with value {attack_value_for_this_round}...")
    ...
    def update_user_setting(self):
        for instance_name, setting_dict in self.user_setting.items():
            ...
            print(f"Modifying data for {instance_name} ...")
            ...
            for instance_id, value_queue in setting_dict.items():
                ...
                print(f"--- User setting: Modifying data for {instance_id} with value {value_for_this_round}...")
    ...
    # Pandapower built-in calculation
    def panda_run_index(self, index):
    ...
    def calculate(self, round):
        print("INFO", f"Calculating this round...")
        # +--------------------------
        # | PANDAPOWER Logics
        # +--------------------------
       ...
       # Update line load percentage from simulation result
        ...
        #added by daisuke for initialization
        self.relays = []
        ...
        #added by Daisuke tentatively for initialization.
        self.transformer_temperature = []
        for i in range(len(Constants.NETWORK_TOPOLOGY["temp_sensor_transformer_mapping"].items())):
            self.transformer_temperature.append(Constants.ROOM_TEMPERATURE)
...
#            transformer_id = Constants.NETWORK_TOPOLOGY["temp_sensor_transformer_mapping"][i]
#            transformer_loading_percent = self.net["trafo"]["i0_percent"][transformer_id]
                #transformer_loading_percent = ppnet["trafo"]["i0_percent"][transformer_id]
                print("TF LOADING: " + str(ppnet["trafo"]["i0_percent"][i]))
...
# linear relationship
                ...
                print(f"Transformer loading percent is {transformer_loading_percent}")
                print(f"Temperature for sensor{i} is {self.transformer_temperature[i]}")
...
#        self.check_temperature()
        ...
        if round > 1:
            ...
            #print("Frequency this round:" + str(self.frequency_this_round))
            for i in range(len(Constants.NETWORK_TOPOLOGY["fm_relay_mapping"].items())):
...
#        self.check_frequency()
    ...
    def write_to_file(self):
        pp.to_json(
            net      = self.net,
            filename = Constants.NETWORK["FILE_PATH"]
        )
